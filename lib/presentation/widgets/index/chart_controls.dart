...
class IndicatorChips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 활성 칩 먼저, 비활성 칩 나중에 (활성 순서 유지)
    final activeKeys = activeIndicators.toList();
    final inactiveKeys = _allIndicators
        .map((e) => e['key']!)
        .where((k) => !activeIndicators.contains(k))
        .toList();
    final orderedKeys = [...activeKeys, ...inactiveKeys];
