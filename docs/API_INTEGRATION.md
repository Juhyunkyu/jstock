...
알파 사이클 앱은 8개의 외부 API를 조합하여 실시간 시세, 차트, 환율, 뉴스, 시장 심리 데이터를 제공합니다. 모든 API는 CORS를 지원하며 프록시 서버 없이 직접 호출합니다.

| API | 역할 | 무료 한도 |
|-----|------|----------|
| **Finnhub** | 실시간 시세, WebSocket | 60회/분 |
| **Twelve Data** | 차트 데이터 (OHLC) + 실시간 환율 | 800회/일 (8회/분) |
| **open.er-api.com** | 환율 (Fallback 1) | 무제한 |
| **Frankfurter** | 환율 (Fallback 2) | 무제한 |
| **한국수출입은행** | 매매기준율/살때/팔때 | 무제한 (API키 필요) |
...
### 3. 환율 시스템 (2026-03-05 업그레이드)

**실시간 환율 (Primary → Fallback 체인)**

| 우선순위 | API | 특징 |
|----------|-----|------|
| Primary | Twelve Data Forex | 실시간 1분 갱신, 5분 캐시 |
...
- [x] 환율 API 연동 (Twelve Data Primary + open.er-api.com + Frankfurter 폴백)
...
| 환율 (실시간) | Twelve Data Forex | ~50회 | 800회/일 (차트와 공유) | 충분 |
...
### 환율
- Primary: Twelve Data Forex (실시간) → Fallback 1: open.er-api.com → Fallback 2: Frankfurter
...
*최종 수정: 2026-03-05 (환율 시스템 업그레이드 — Twelve Data 실시간 + 한국수출입은행 매매기준율)*
